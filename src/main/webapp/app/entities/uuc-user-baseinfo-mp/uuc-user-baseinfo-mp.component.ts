...
export class UucUserBaseinfoMpComponent implements OnInit, OnDestroy {
  ...
  loadPage(page?: number, dontNavigate?: boolean): void {
    ...
    this.uucUserBaseinfoService
      ...
      .subscribe(
        (res: HttpResponse<IUucUserBaseinfoMp[]>) => this.onSuccess(res.body, res.headers, pageToLoad, !dontNavigate),
        () => this.onError()
      );
  }
  ...
  registerChangeInUucUserBaseinfos(): void {
    this.eventSubscriber = this.eventManager.subscribe('uucUserBaseinfoListModification', () => this.loadPage());
  }
  ...
  protected onSuccess(data: IUucUserBaseinfoMp[] | null, headers: HttpHeaders, page: number, navigate: boolean): void {
    this.totalItems = Number(headers.get('X-Total-Count'));
    this.page = page;
    if (navigate) {
      this.router.navigate(['/uuc-user-baseinfo-mp'], {
        queryParams: {
          page: this.page,
          size: this.itemsPerPage,
